...
<div class="border-bottom border-secondary py-3 px-3">
  <nav aria-label="breadcrumb">
    <div class="ps-container-breadcrumb ">
      <ol class="breadcrumb mb-0 text-truncate"> {FORUMS_POSTS_BREADCRUMBS} </ol>
    </div>
  </nav>
</div>
<div class="min-vh-50 px-2 px-md-3 py-4">
  <div class="row justify-content-center">
    <div class="col-12 container-3xl">
      ...
      <div class="card mb-4">
        <div class="card-body">
          <h2 class="polls h5 mb-3">{POLLS_TITLE}</h2> {POLLS_FORM}
        </div>
      </div>
      <!-- END: POLLS_VIEW -->
      ...
      <div class="card mb-4">
        <div class="card-body">
          <div class="row g-3 align-items-center">
            ...
            <div class="col-md-9 col-12">
              <form id="movetopic" action="{FORUMS_POSTS_MOVE_URL}" method="post" class="d-flex flex-wrap gap-2 align-items-center"> {FORUMS_POSTS_MOVEBOX_SELECT} <span class="small text-muted">{FORUMS_POSTS_MOVEBOX_KEEP} {PHP.L.forums_keepmovedlink}</span>
      ...
      <!-- END: FORUMS_POSTS_ADMIN --> {FILE "{PHP.cfg.themes_dir}/{PHP.cfg.defaulttheme}/warnings.tpl"} <div class="row">
        <!-- IF {FORUMS_POSTS_TOPIC_XTR_FLD_YOUTUBE} -->
        <div class="col-md-6 col-12 mb-2 mb-md-0">
          <button type="button" class="video-btn btn btn-danger mb-3" id="programmaticBtn"> {FORUMS_POSTS_TOPIC_XTR_FLD_YOUTUBE_TITLE} </button>
        </div>
        <!-- ENDIF -->
        <!-- IF {FORUMS_POSTS_TAGS} -->
        <div class="col-md-6 mb-3 col-12"> {FORUMS_POSTS_TAGS} </div>
        <!-- ENDIF -->
      </div>
      ...
      <div class="card mb-4">
        <div class="card-header" style="background-color: var(--bs-sidebar-bg)">
          <div class="row g-2 align-items-center">
            <div class="col-auto d-none d-lg-block"> {PHP.L.Author} </div>
            <div class="col">
              <div class="d-flex flex-wrap justify-content-lg-end gap-2 small text-muted">
                ...
                <span>{PHP.cfg.separator}</span> {FORUMS_POSTS_ROW_CREATION}
                <!-- IF {FORUMS_POSTS_ROW_POSTERIP} -->
                <span>{PHP.cfg.separator}</span> {FORUMS_POSTS_ROW_POSTERIP}
                <!-- ENDIF -->
                <!-- IF {FORUMS_POSTS_ROW_QUOTE} -->
                <span>{PHP.cfg.separator}</span> {FORUMS_POSTS_ROW_QUOTE}
                <!-- ENDIF -->
                <!-- IF {FORUMS_POSTS_ROW_EDIT} -->
                <span>{PHP.cfg.separator}</span> {FORUMS_POSTS_ROW_EDIT}
                <!-- ENDIF -->
                <!-- IF {FORUMS_POSTS_ROW_DELETE} -->
                <span>{PHP.cfg.separator}</span> {FORUMS_POSTS_ROW_DELETE}
                <!-- ENDIF --> {FORUMS_POSTS_ROW_BOTTOM}
              </div>
            </div>
          </div>
        </div>
        <div class="card-body {FORUMS_POSTS_ROW_ODDEVEN}">
          <div class="row align-items-start">
            <div class="col-12 border-bottom">
              <div class="row mb-3 ">
                <div class="col-lg-3"> {FORUMS_POSTS_ROW_ANCHORLINK} <div class="row">
                    <div class="col-auto">
                      <div class="">
                    ...
                    <div class="col ">
                      <div class="ms-lg-2 mt-2 mt-lg-0">
                        <h4 class="h6 mb-2">{FORUMS_POSTS_ROW_USER_NAME}</h4>
                        <p class="small mb-0">{FORUMS_POSTS_ROW_USER_MAIN_GROUP_TITLE}</p>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="col-lg-2 text-center text-lg-start">
                  <div class="row align-items-center">
                    ...
                    <div class="col d-flex align-items-center">
                      <p class="small mb-0">{PHP.L.forums_posts}: {FORUMS_POSTS_ROW_USER_POSTCOUNT}</p>
                    </div>
                  </div>
                </div>
                <div class="col-lg-7">
                  <div class="small text-muted">
                    <pre>{FORUMS_POSTS_ROW_USER_TEXT|strip_tags($this)|cot_string_truncate($this, '120')}</pre>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="col-12 mb-3">
              <!-- IF {FORUMS_POSTS_ROW_ID|att_count('forums',$this)} > 0 --> {FORUMS_POSTS_ROW_ID|att_count('forums',$this)}/{FORUMS_POSTS_ROW_ID|att_count('forums',$this,'','images')}/{FORUMS_POSTS_ROW_ID|att_count('forums',$this,'','files')} <div class="col-12 mb-3">
                <!-- IF {FORUMS_POSTS_ROW_ID|att_count('forums', $this, '', 'images')} > 0 --> {FORUMS_POSTS_ROW_ID|att_display('forums',$this,'','attacher.display.forums.postsrow','images')}
                <!-- ENDIF -->
                <!-- IF {FORUMS_POSTS_ROW_ID|att_count('forums', $this, '', 'files')} > 0 --> {FORUMS_POSTS_ROW_ID|att_downloads('forums',$this)}
                <!-- ENDIF -->
              </div>
              <!-- ENDIF -->
              ...
              <div class="col-12 mb-3">
                ...
                <div class="input-group"> {FORUMS_POSTS_ROW_ID|att_filebox('forums', $this)} </div>
              </div>
              <!-- ENDIF -->
            </div>
            <!-- ENDIF -->
      ...
      <div class="card mt-4">
        <div class="card-body">
          <form action="{FORUMS_POSTS_NEWPOST_SEND}" method="post" name="newpost">
            <div class="row g-3">
              <div class="col-12"> {FORUMS_POSTS_NEWPOST_TEXT}
                <!-- IF {PHP|cot_module_active('pfs')} AND !{PHP|cot_module_active('files')} -->
                <!-- IF {FORUMS_POSTS_NEWPOST_PFS} --> {FORUMS_POSTS_NEWPOST_PFS}
                <!-- ENDIF -->
                <!-- IF {FORUMS_POSTS_NEWPOST_SFS} -->
                <span class="spaced">{PHP.cfg.separator}</span> {FORUMS_POSTS_NEWPOST_SFS}
                <!-- ENDIF -->
                <!-- ENDIF -->
...
<div class="col-12 col-md-10 mx-auto"> {FILE "{PHP.cfg.themes_dir}/{PHP.cfg.defaulttheme}/inc/mskin.tpl"} </div>
<!-- ENDIF -->
    <script>
