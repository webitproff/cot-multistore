...
if ($a == 'cleanup') {
    ...
    if (cot_module_active('mstore')) {
        // Remove unused page attachments
        require_once cot_incfile('mstore', 'module');

        $condition = "LEFT JOIN $db_mstore ON $db_attacher.att_item = $db_mstore.msitem_id
		WHERE $db_attacher.att_area = 'mstore' AND $db_mstore.msitem_id IS NULL";
