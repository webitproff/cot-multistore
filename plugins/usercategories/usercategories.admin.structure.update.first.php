...
if (!empty($editconfig) && $n == 'usercategories')
{
    ...
    {
        // безопасная проверка фильтра перед sizeof
        $filter = isset($cot_import_filters[$key]) ? $cot_import_filters[$key] : array();

        $data = cot_import($key, 'P', sizeof($filter) ? $key : 'NOC');
