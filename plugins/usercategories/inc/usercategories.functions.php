...
function cot_usercategories_tree($chosen = '', $parent = '', $template = '', $level = 0)
{
    ...
    if (!empty($chosen) && is_string($chosen)) {
        $chosen = explode(',', $chosen);
    } elseif (empty($chosen)) {
        $chosen = [];
    }
...
function cot_usercategories_chosen($chosen, $template = '')
{
    global $structure;
    $t1 = new XTemplate(cot_tplfile(['usercategories', 'chosen', $template], 'plug'));
    if (!is_array($chosen)) {
        $chosen = !empty($chosen) ? explode(',', $chosen) : [];
    }
    if (!empty($chosen[0]) && isset($structure['usercategories'][$chosen[0]])) {
        $cat = $chosen[0];
        $t1->assign([
            "CAT_CHOSEN_CODE" => $cat,
            "CAT_CHOSEN_TITLE" => htmlspecialchars($structure['usercategories'][$cat]['title']),
            "CAT_CHOSEN_DESC" => $structure['usercategories'][$cat]['desc'],
        ]);
        $t1->parse("MAIN.CAT_CHOSEN");
    }
    $t1->parse("MAIN");
    return $t1->text("MAIN");
}