...
function cot_generate_mstoretags(
	...
	if ($item_data['msitem_id'] > 0 && !empty($item_data['msitem_title'])) {
		...
		if (!empty($item_data['msitem_url']) && $item_data['msitem_file']) {
			$dotpos = mb_strrpos($item_data['msitem_url'], ".") + 1;
			$type = mb_strtolower(mb_substr($item_data['msitem_url'], $dotpos, 5));
			$item_data['msitem_fileicon'] = cot_rc('mstore_icon_file_path', ['type' => $type]);
			if (!file_exists($item_data['msitem_fileicon'])) {
				$item_data['msitem_fileicon'] = cot_rc('mstore_icon_file_default');
			}
			$item_data['msitem_fileicon'] = cot_rc('mstore_icon_file', ['icon' => $item_data['msitem_fileicon']]);

        } else {
			$item_data['msitem_fileicon'] = '';
		}
		...
		$item_data['msitem_status'] = cot_mstore_status(
			...
			$item_data['msitem_begin'],
			$item_data['msitem_expire']
		);

        $haveFile = Cot::$L['No'];
        $item_data['msitem_file'] = (int) $item_data['msitem_file'];
        if ($item_data['msitem_file'] == 1) {
            $haveFile = Cot::$L['Yes'];
        } elseif ($item_data['msitem_file'] == 2) {
        ...
        $temp_array = [
            ...
            'CAT_ICON_SRC' => isset($structure['mstore'][$item_data['msitem_cat']]['icon'])
                ...
                : '',
			'KEYWORDS' => (isset($item_data['msitem_keywords']) && $item_data['msitem_keywords'] != '')
                ? htmlspecialchars($item_data['msitem_keywords'])
                : '',
			...
			'BEGIN' => cot_date($date_format, $item_data['msitem_begin']),
			'EXPIRE' => cot_date($date_format, $item_data['msitem_expire']),
			'UPDATED' => cot_date($date_format, $item_data['msitem_updated']),
            'CREATED_STAMP' => $item_data['msitem_date'],
			'BEGIN_STAMP' => $item_data['msitem_begin'],
			'EXPIRE_STAMP' => $item_data['msitem_expire'],
			'UPDATED_STAMP' => $item_data['msitem_updated'],
			'FILE' => $haveFile,
			'FILE_URL' => !empty($item_data['msitem_url'])
                ? cot_url('mstore', ['c' => $item_data['msitem_cat'], 'id' => $item_data['msitem_id'], 'a' => 'dl'])
                : '',
			'FILE_SIZE' => $item_data['msitem_size'] / 1024, // in KiB; deprecated but kept for compatibility
			'FILE_SIZE_BYTES' => $item_data['msitem_size'],
			'FILE_SIZE_READABLE' => cot_build_filesize($item_data['msitem_size'], 1),
			'FILE_ICON' => $item_data['msitem_fileicon'],
			'FILE_DOWNLOADS' => $item_data['msitem_filecount'],
			'FILE_DOWNLOADS_TIMES' => cot_declension($item_data['msitem_filecount'], $Ls['Times']),
			'FILE_NAME' => !empty($item_data['msitem_url']) ? basename($item_data['msitem_url']) : '',
			'HITS' => $item_data['msitem_count'],
            'ADMIN' => $admin_rights
                ? cot_rc('list_row_admin', ['unvalidate_url' => $unvalidate_url, 'edit_url' => $edit_url])
                : '',
			'NOT_AVAILABLE' => ($item_data['msitem_begin'] > Cot::$sys['now'])
                ? Cot::$L['mstore_notavailable'] . cot_build_timegap(Cot::$sys['now'], $item_data['msitem_begin'])
                : '',
		];
 ...
 * @param int $msitem_begin
 * @param int $msitem_expire
 * @return string 'draft', 'pending', 'approved', 'published' or 'expired'
 */
function cot_mstore_status($msitem_state, $msitem_begin, $msitem_expire)
{
	global $sys;

	if ($msitem_state == 0) {
		if ($msitem_expire > 0 && $msitem_expire <= $sys['now']) {
			return 'expired';
		} elseif ($msitem_begin > $sys['now']) {
			return 'approved';
		}
		return 'published';
	}
...
function cot_mstore_import($source = 'POST', $ritem = [], $auth = [])
{
	...
	$ritem['msitem_file']     = intval(cot_import('rmsitemfile', $source, 'INT'));
	$ritem['msitem_url']      = cot_import('rmsitemurl', $source, 'TXT', 255);
	$ritem['msitem_size']     = (int) cot_import('rmsitemsize', $source, 'INT');
	$ritem['msitem_file']     = ($ritem['msitem_file'] == 0 && !empty($ritem['msitem_url'])) ? 1 : $ritem['msitem_file'];
	...
	$ritem['msitem_begin']    = (int) cot_import_date('rmsitembegin');
	$ritem['msitem_expire']   = (int) cot_import_date('rmsitemexpire');
	$ritem['msitem_expire']   = ($ritem['msitem_expire'] <= $ritem['msitem_begin']) ? 0 : $ritem['msitem_expire'];
	$ritem['msitem_updated']  = $sys['now'];

	$ritem['msitem_keywords'] = cot_import('rmsitemkeywords', $source, 'TXT', 255);
	...
	if ($auth['isadmin'] && isset($ritem['msitem_ownerid'])) {
		...
		$ritem['msitem_filecount'] = cot_import('rmsitemfilecount', $source, 'INT');
	} else {
