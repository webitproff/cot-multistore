...
class MstoreControlService
{
    ...
    function delete(int $id, array $itemData = [])
    {
        ...
        if (empty($itemData)) {
            $itemData = Cot::$db->query('SELECT * FROM ' . Cot::$db->mstore . ' WHERE msitem_id = ?', $id)->fetch();
        }
        if (empty($itemData)) {
            return false;
        }
        ...
        try {
            ...
            /* === Hook === */
            foreach (cot_getextplugins('mstore.delete.first') as $pl) {
                include $pl;
            }
            /* ===== */
            ...
            ItemService::getInstance()->onDelete(MstoreDictionary::SOURCE_MSTORE, $id);
