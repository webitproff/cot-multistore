...
if (!empty($c) && isset(Cot::$structure['mstore'][$c])) {
	...
	Cot::$cfg['maintitle'] = 'Товары и поставщики';
...
if ($c === 'unvalidated') {
    ...
    $where['cat'] = !empty($c) && isset(Cot::$structure['mstore'][$c])
        ? 'msitem_cat = ' . Cot::$db->quote($c)
        : "msitem_cat IN (SELECT structure_code FROM $db_structure WHERE structure_area = 'mstore')";
    if (Cot::$usr['isadmin']) {
        $where['state'] = '1'; // Админы видят все товары
    } else {
