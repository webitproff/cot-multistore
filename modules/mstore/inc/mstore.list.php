...
/**
 * Store item list
 * filename mstore.list.php
 * @package Mstore
 * @copyright (c) webitproff
 * @license BSD
 */
// Документация: описание модуля Mstore, его назначение, автор и лицензия

use cot\modules\mstore\inc\MstoreDictionary;
// Подключение пространства имен для использования класса MstoreDictionary
...
// Импорт параметра сортировки 's' (имя поля без префикса 'msitem_') из GET-запроса, ожидая алфавитное значение
...
// Импорт массива имен полей для фильтрации 'ord' (без префикса 'msitem_') из GET-запроса, ожидая массив
...
// Установка максимального количества элементов на страницу из конфигурации по умолчанию для модуля Mstore
...
if ($maxItemRowsPerPage <= 0) {
    ...
    // Установка значения из общей конфигурации модуля Mstore
}
...
if ($c === 'all' || $c === 'system') {
    ...
    // Установка прав доступа для любой категории модуля Mstore
...
foreach (cot_getextplugins('mstore.list.first') as $pl) {
    // Цикл по плагинам, зарегистрированным для хука 'mstore.list.first'
    include $pl;
...
// Проверка, указан ли код категории и существует ли она в структуре модуля Mstore
if (!empty($c) && isset(Cot::$structure['mstore'][$c])) {
    ...
    $cat['title'] = $cfg['mstore']['mstorelist_default_title'] ?: (Cot::$L['mstore_all_items'] ?: 'All Items');
    // Установка описания категории по умолчанию, если категория не указана
    $cat['desc'] = $cfg['mstore']['mstorelist_default_desc'] ?: (Cot::$L['mstore_all_items_desc'] ?: 'All available store items');
...
if ($c === 'unvalidated') {
    ...
    if (!empty($c) && isset(Cot::$structure['mstore'][$c])) {
        $catsub = cot_structure_children('mstore', $c, true);
        $catsub[] = $c; // добавляем саму категорию
        $catsub_quoted = array_map([Cot::$db, 'quote'], $catsub);
        $where['cat'] = "msitem_cat IN (" . implode(',', $catsub_quoted) . ")";
    } else {
    ...
    if (Cot::$usr['isadmin']) {
        ...
        $where['state'] = '(msitem_state = ' . MstoreDictionary::STATE_PUBLISHED
            . ' OR (msitem_state = ' . MstoreDictionary::STATE_PENDING
            . ' AND msitem_ownerid = ' . Cot::$usr['id'] . '))';
    }
}
...
if (!Cot::$usr['isadmin'] && $c !== 'unvalidated' && $c !== 'saved_drafts') {
    ...
    $where['date'] = "msitem_date <= UNIX_TIMESTAMP()";
}
...
if (!empty($sq)) {
    ...
    $where['search'] = "(msitem_title LIKE $sq_escaped OR msitem_text LIKE $sq_escaped)";
}

// Проверяем, существует ли поле для сортировки в таблице mstore
...
// Формируем базовый путь хлебных крошек с главной страницей и модулем Mstore
$mstoreCatpPath = [
    [cot_url('index'), $L['Main']],
    [cot_url('mstore'), $L['mstore_Mstore']]
];

// Если категория указана, добавляем путь текущей категории в хлебные крошки
if (!empty($c)) {
    $mstoreCatpPath = array_merge($mstoreCatpPath, cot_structure_buildpath('mstore', $c));
}

// Формируем HTML-код хлебных крошек на основе массива пути
$msCatpPath = cot_breadcrumbs($mstoreCatpPath, $cfg['homebreadcrumb'], true);
...
foreach (cot_getextplugins('mstore.list.query') as $pl) {
    // Цикл по плагинам, зарегистрированным для хука 'mstore.list.query'
    include $pl;
    // Подключение каждого плагина
}
/* ===== */
...
if ($o && $p) {
    ...
    foreach ($filters as $key => $val) {
        ...
        // Проверка, что ключ и значение заданы и поле существует в таблице mstore
...
// Подключение хуков для плагинов, зарегистрированных на событие 'mstore.list.main'
...
$arrows = [];
// Перебор дополнительных полей таблицы mstore и стандартных полей (title, key, date, author, owner, count, filecount)
...
/* === Hook === */
// Подключение хуков для плагинов, зарегистрированных на событие 'mstore.list.rowcat.first'
...
/* === Hook - Part1 : Set === */
// Получение списка плагинов для хука 'mstore.list.rowcat.loop'
...
foreach ($subcat as $x) {
    ...
    // Подключение хуков для плагинов, зарегистрированных на событие 'mstore.list.rowcat.loop'
    foreach ($extp as $pl) {
        // Включение каждого плагина
        include $pl;
    }
...
// Инициализация хуков для плагинов, зарегистрированных на событие 'mstore.list.loop'
/* === Hook - Part1 : Set === */
$extp = cot_getextplugins('mstore.list.loop');
// Получение списка плагинов для хука 'mstore.list.loop'
/* ===== */
// Завершение инициализации хуков
...
foreach (cot_getextplugins('mstore.list.before_loop') as $pl) {
    // Цикл по плагинам, зарегистрированным для хука 'mstore.list.before_loop'
    include $pl;
    // Включение каждого плагина
}
/* ===== */
...
if (!$sqllist_rowset_other) {
    ...
    foreach ($sqllist_rowset as $item) {
        ...
        /* === Hook - Part2 : Include === */
        foreach ($extp as $pl) {
            // Включение каждого плагина из хука 'mstore.list.loop'
            include $pl;
        }
        /* ===== */
        // Завершение блока хуков
...
// Подключение хуков для плагинов, зарегистрированных на событие 'mstore.list.tags'
/* === Hook === */
