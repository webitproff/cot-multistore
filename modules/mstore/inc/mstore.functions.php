...
/**
 * Renders structure dropdown forcot_mstore_selectbox_structure_select2 with Select[](https://select2.org/) support and indented subcategories
 *
 ...
 * @return string
 */
function cot_mstore_selectbox_structure_select2(
    $extension,
    $check,
    $name,
    $subcat = '',
    ...
    $blacklist_cfg = Cot::$cfg['mstore']['mstoreblacktreecatspage'] ?? '';
 ...
 * @return string
 */
function cot_mstore_selectcat_select2($check, $name, $subcat = '', $hideprivate = true)
{
    ...
    $structure['mstore'] = is_array($structure['mstore']) ? $structure['mstore'] : [];
    ...
    // Перебираем все категории в разделе 'mstore'
    foreach ($structure['mstore'] as $i => $x) {
        // Пропускаем категории, которые находятся в черном списке
 ...
 * @return string
 */
function cot_mstore_selectcat($check, $name, $subcat = '', $hideprivate = true)
{
    global $structure;
    ...
    $structure['mstore'] = (is_array($structure['mstore'])) ? $structure['mstore'] : array();
    ...
    foreach ($structure['mstore'] as $i => $x)
    {
        ...
        if (cot_auth('mstore', $i, 'R') && $i != 'all' && $display) {
...
function cot_generate_mstoretags(
	...
	if ($item_data['msitem_id'] > 0 && !empty($item_data['msitem_title'])) {
        ...
        $temp_array = [
			...
			'TEXT' => $text,
			'TEXT_SHORT' => cot_cutstring(strip_tags($item_data['msitem_text']), 250),
			'TEXT_CUT' => $text_cut,
			'TEXT_IS_CUT' => $cutted,
			...
			'OWNER_NAME' => (isset($item_data['user_name']) && $item_data['user_name'] != '')
                ...
                : '',
			'COSTDFLT' => (floor($item_data['msitem_costdflt']) != $item_data['msitem_costdflt']) ? number_format($item_data['msitem_costdflt'], '2', '.', ' ') : number_format($item_data['msitem_costdflt'], '0', '.', ' '),
...
function cot_mstore_import($source = 'POST', $ritem = [], $auth = [])
{
	...
	$ritem['msitem_costdflt'] = cot_import('rmsitemcostdflt', $source, 'NUM');
