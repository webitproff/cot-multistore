...
<div class="mb-3">
	<div class="row g-2">
		<div class="col-12 col-lg-3">
			<a href="{ADMIN_MSTORE_URL_CONFIG}" class="btn btn-outline-secondary w-100">
			<i class="fa-solid fa-gear me-1"></i>{PHP.L.Configuration} </a>
		</div>
		<div class="col-12 col-lg-3">
			<a href="{ADMIN_MSTORE_URL_STRUCTURE}" class="btn btn-outline-secondary w-100">
			<i class="fa-solid fa-list-ul me-1"></i>{PHP.L.Categories} </a>
		</div>
		<div class="col-12 col-lg-3">
			<a href="{ADMIN_MSTORE_URL_EXTRAFIELDS}" class="btn btn-outline-secondary w-100">
			<i class="fa-solid fa-table-columns me-1"></i>{PHP.L.Extrafields} </a>
		</div>
		<div class="col-12 col-lg-3">
			<a href="{ADMIN_MSTORE_URL_ADD}" class="btn btn-outline-primary w-100">
			<i class="fa-solid fa-plus me-1"></i>{PHP.L.mstore_addtitle} </a>
		</div>
	</div>
</div> 
...
<div class="card filter-section p-3 mb-4" style="border: 5px var(--bs-dark-border-subtle) solid">
	<!-- IF {ADMIN_MSTORE_TOTALDBITEMS} -->
	<form name="form_valqueue" method="get" action="{ADMIN_MSTORE_SEARCH_ACTION_URL}" class="mb-3">
		<!-- IF !{PHP|cot_plugin_active('urleditor')} OR {PHP.cfg.plugin.urleditor.preset} != 'handy' -->
		<input type="hidden" name="m" value="mstore" />
		<!-- ENDIF -->
		<div class="row g-2 align-items-end">
			<div class="col-12 col-lg-3 d-flex flex-column h-100">
				<label class="form-label">{PHP.L.Search}</label>
				<div class="flex-grow-1">{ADMIN_MSTORE_SEARCH_SQ}</div>
			</div>
			<div class="col-12 col-lg-3 d-flex flex-column h-100">
				<label class="form-label">{PHP.L.Category}</label>
				<div class="flex-grow-1 filterSelect">{ADMIN_MSTORE_SEARCH_CAT_SELECT2}</div>
			</div>
			<div class="col-12 col-lg-3 d-flex flex-column h-100">
				<label class="form-label">{PHP.L.cotcp_select_filter_options}</label>
				<div class="flex-grow-1">{ADMIN_MSTORE_FILTER}</div>
			</div>
			<!-- IF {TOTAL_ENTRIES} > 1 -->
			<div class="col-12 col-lg-3 d-flex flex-column h-100">
				<label class="form-label">{PHP.L.adm_sort}</label>
				<div class="flex-grow-1">{ADMIN_MSTORE_ORDER}</div>
			</div>
			<div class="col-12 col-lg-3 d-flex flex-column h-100">
				<label class="form-label">{PHP.L.cotcp_select_filter_sorting_direction}</label>
				<div class="flex-grow-1">{ADMIN_MSTORE_WAY}</div>
			</div>
			<!-- ENDIF -->
			<div class="col-12 col-lg-3 d-flex flex-column h-100">
			...
			<div class="col-12 col-lg-3 d-flex flex-column h-100">
				<a class="btn btn-outline-danger w-100 mt-auto" href="{PHP|cot_url('admin','m=mstore')}"><i class="fa-solid fa-broom me-1"></i>{PHP.L.Prune} </a>
			</div>
		</div>
	</form>
	<!-- IF {ADMIN_MSTORE_SEARCH_RESULT_MSG} -->
	<div class="alert alert-info my-2" role="alert">
		{ADMIN_MSTORE_SEARCH_RESULT_MSG}
	</div>
	<!-- ENDIF -->
	<!-- ENDIF -->
</div>
<div class="mb-3">
	<form id="form_valqueue" name="form_valqueue" method="post" action="{ADMIN_MSTORE_FORM_URL}">
			...
			<div class="list-group-item list-group-item-action">
				<div class="row align-items-center">
					<div class="col-1 text-center">
						<input type="checkbox" name="s[{ADMIN_MSTORE_ID}]" class="form-check-input checkbox" />
					</div>
					<div class="col-1">{ADMIN_MSTORE_ID}</div>
					<div class="col-md-2">{ADMIN_MSTORE_LOCAL_STATUS}</div>
					<div class="col-md-5">
						<div id="mor_{PHP.ii}" class="mor_info_on_off overflow-x-auto" style="max-width: 675px;">
							<span class="fw-bold" style="cursor: pointer;">{ADMIN_MSTORE_TITLE}</span>
							<!-- IF {ADMIN_MSTORE_DESCRIPTION} -->
							<div class="text-muted small">{ADMIN_MSTORE_DESCRIPTION}</div>
							<!-- ENDIF -->
							<div class="moreinfo collapse">
								<hr class="my-2" />
								<strong>{ADMIN_MSTORE_UPDATED}</strong> 
								<!-- IF {ADMIN_MSTORE_TEXT} -->
								<div class="mt-2">
									<strong>{PHP.L.Text}:</strong>
									<div>{ADMIN_MSTORE_TEXT}</div>
								</div>
								<!-- ENDIF -->
							</div>
						</div>
					...
					<div class="col-md-3">
						<div class="d-flex flex-wrap gap-1">
							<!-- IF {PHP.row.msitem_state} == 1 -->
							<a href="{ADMIN_MSTORE_URL_FOR_VALIDATED}" class="btn btn-sm btn-outline-success confirmLink">
							<i class="fa-solid fa-check me-1"></i>{PHP.L.Validate} </a>
							<!-- ENDIF -->
							<a href="{ADMIN_MSTORE_URL_FOR_DELETED}" class="btn btn-sm btn-outline-danger confirmLink">
							<i class="fa-solid fa-trash me-1"></i>{PHP.L.Delete} </a>
							<a href="{ADMIN_MSTORE_ID_URL}" target="_blank" class="btn btn-sm btn-primary">
							<i class="fa-solid fa-eye me-1"></i>{PHP.L.Open} </a>
							<a href="{ADMIN_MSTORE_URL_FOR_EDIT}" target="_blank" class="btn btn-sm btn-outline-secondary">
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		submitButtons.forEach(function (elem) {
			elem.addEventListener('click', function (e) {
				...
				switch (this.value) {
					case 'delete':
					message = '{PHP.L.mstore_confirm_delete}';
					break;
					case 'validate':
					message = '{PHP.L.mstore_confirm_validate}';
					break;
				}
