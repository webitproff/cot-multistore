...
if ($filter == 'all') {
    ...
    $sqlwhere = 'msitem_expire > 0 AND msitem_expire < ' . (int) Cot::$sys['now'] . " AND msitem_title IS NOT NULL AND msitem_title != ''";
} else {
    $sqlwhere = "msitem_title IS NOT NULL AND msitem_title != ''";
}
...
if (!empty($sq)) {
    ...
    $sqlwhere .= " AND (msitem_title LIKE :sq OR msitem_text LIKE :sq)";
    $params['sq'] = "%$sq%";
}

// Фильтр по категории (учитываем подкатегории)
$catsub = cot_structure_children('mstore', !empty($c) ? $c : '');
if (!empty($catsub)) {
...
$totalitems = Cot::$db->query(
    'SELECT COUNT(*) FROM ' . Cot::$db->mstore . ' WHERE ' . $sqlwhere,
    $params
)->fetchColumn();
...
$ii = 0;
$sqllist_rowset = $sql_mstore->fetchAll(); // Сохраняем результат для подсчёта
...
foreach ($sqllist_rowset as $row) {
    ...
    $categoryPath = isset(Cot::$structure['mstore'][$row['msitem_cat']]['path'])
        ? Cot::$structure['mstore'][$row['msitem_cat']]['path']
        : (isset(Cot::$structure['mstore'][$row['msitem_cat']]['code']) ? Cot::$structure['mstore'][$row['msitem_cat']]['code'] : $row['msitem_cat']);

    // НЕ затираем $urlParams для админки
    $itemUrlParams = ['c' => $categoryPath];
    if (!empty($row['msitem_alias'])) {
        $itemUrlParams['al'] = $row['msitem_alias'];
    } else {
        $itemUrlParams['id'] = $row['msitem_id'];
    }
    $row['item_pageurl'] = cot_url('mstore', $itemUrlParams);
...
$t->assign([
    ...
    'ADMIN_MSTORE_SEARCH_ACTION_URL' => cot_url('admin', $urlParams, '', true), // URL формы поиска
    'ADMIN_MSTORE_SEARCH_SQ' => cot_inputbox(
        'text',
        'sq',
    ...
    "ADMIN_MSTORE_SEARCH_CAT_SELECT2" => cot_mstore_selectcat_select2($c, 'c'),
	// Поле ввода поиска
    'ADMIN_MSTORE_SEARCH_RESULT_MSG' => $searchMsg,
]);
